...
the hessian's leading (largest) eigen values-vectors (or their approximation.\n\
...
void LoadEigen(Allocator *allocator, GradientMachine *machine, int n_eigen, Vec **eigenvals, Mat **eigenvecs);
...
int main(int argc, char **argv)
{
  ...
  Vec **eigenvals = (Vec**) allocator->alloc(sizeof(Vec*)*csae->params->n_data);
  Mat **eigenvecs = (Mat**) allocator->alloc(sizeof(Mat*)*csae->params->n_data);

  LoadEigen(allocator, csae, flag_n_eigen, eigenvals, eigenvecs);
...
void LoadEigen(Allocator *allocator, GradientMachine *machine, int n_eigen, Vec **eigenvals, Mat **eigenvecs)
{
  for (int i=0; i<machine->params->n_data; i++)  {
    ...
    eigenvals[i] = new(allocator) Vec(n_eigen);
    eigenvecs[i] = new(allocator) Mat(n_eigen, machine->params->size[i]);
    ...
    while (getline(fd_eigenvecs, line))  {
      ...
      while (tokens >> value) {
        eigenvecs[i]->ptr[n_vecs][n_val] = value;
        n_val++;
      }
      assert (n_val == machine->params->size[i]);
      n_vecs++;
    }
...
Vec **eigenvals, Mat **eigenvecs, real stepsize, bool eigen_step)
{
  for (int i=0; i<machine->params->n_data; i++)  {
    ...
    for (int j=0; j<machine->params->size[i]; j++)
      ptr[j] += stepsize * eigenvecs[i]->ptr[id_eigen][j];
  }
}
                                ...
                                bool eigenstep)
{
  ...
  measurer_filename << "hessian/" << data_label << "_nll_eigen" << id_eigen << ".txt";
  ...
  measurer_filename << "hessian/" << data_label << "_class_eigen" << id_eigen << ".txt";
